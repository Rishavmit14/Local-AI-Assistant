...
export function CanonicalProgress({ openLearn }: { openLearn: () => void }) {
  ...
  return <section className="canonical-projection" aria-label="Canonical learning progress">
    ...
    <div className="canonical-progress-columns">
      ...
      <section><h3>Career readiness</h3><strong>{progress.readiness.status.replaceAll("_", " ")}</strong><p>Interview: {progress.readiness.interview_status.replaceAll("_", " ")} · Portfolio: {progress.readiness.portfolio_status.replaceAll("_", " ")}</p><small>{progress.readiness.independent_competencies}/{progress.readiness.total_competencies} competencies at independent application · {progress.readiness.correct_interview_responses} correct no-help interview responses · {progress.readiness.published_artifacts} published artifacts</small>{progress.readiness.blockers.length?<ol className="canonical-record-list">{progress.readiness.blockers.map((blocker)=><li key={blocker}>{blocker}</li>)}</ol>:<p>All explicit evidence gates are currently supported.</p>}</section>
