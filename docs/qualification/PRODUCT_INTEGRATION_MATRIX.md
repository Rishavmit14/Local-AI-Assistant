...
| 44. GitHub portfolio evidence | qualified CF evidence delegates to authenticated promotion gateway | no natural publish command | PROJECTS reviews and approves; publication endpoint is not exposed in UI | CF + task/Git history | explicit owner approval plus `GITHUB_WRITE`; linked objectives require exact repository/task and succeeded state | PROJECTS approval, then authenticated gateway | deterministic publication, mismatch, nonterminal and retry coverage | INTEGRATED | final owner-visible publish action and real E2E remain | surface existing authenticated publication without bypass | 43, 45 | CF-E2E-008 |
