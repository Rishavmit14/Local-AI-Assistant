...
| 26. Career Forge core | `CareerForgeService`, graph/missions/evidence | typed information/invocation adapter and active-session handoff | journey panel | CF SQLite | Learner Twin writes deterministic; model cannot mutate it | ask Friday to open/teach/resume/status | real text Learner Twin handoff plus physical wake/open/alias/stop passed | USABLE | Practice Lab and advanced workspaces remain absent | extend only through bounded next slice | 9, 10 | E2E-011/012 |
| 27. Career Forge LEARN | mission brief/loop | canonical mission starts only on explicit teach request; tutor context shares Friday conversation | mission start/resume text | CF SQLite | owner pace; no model mastery/evidence writes | ask Friday to teach ML | real local tutor handoff and session continuity passed | USABLE | no Practice Lab/attempt workspace | bounded interactive learning workspace later | 26 | E2E-012 |
...
| 31. Career Forge mission resume | `resume`, resume endpoint | natural typed resume handoff | panel reads current mission | CF SQLite | owner-controlled pace | ask Friday to resume the Career Forge mission | real persisted mission routed through text and physical conversation | USABLE | complete post-restart owner lesson proof remains | retain canonical resume boundary | 26 | CF-E2E-001 |
