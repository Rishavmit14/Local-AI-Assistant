...
| 38. Arbitrary code selection context | bounded contextual tutor accepts explicit selected code | explicit one-shot mission context | CodeMirror selection action | transient request only | explicit owner selection; untrusted bounded text; no learner mutation | select any code region, then Explain selection | deterministic API/frontend coverage; live owner-path qualification remains | INTEGRATED | real owner selection/answer qualification remains | retain exact-selection boundary | 23, 36 | CF-E2E-003 |
...
| 41. Interview mode | durable two-question no-help interview and evidence binding | same local assessment boundary | connected Interview workspace with restart resume | CF SQLite | no assistance; no auto-mastery/readiness; evidence-derived projection only | begin no-help interview | deterministic API/frontend/restart coverage | INTEGRATED | final live conversational interview qualification remains | retain session-bound evidence and readiness truth | 27, 34 | CF-E2E-009 |
