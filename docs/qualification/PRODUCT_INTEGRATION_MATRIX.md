...
| 43. Project generation/integration | project-linked mission can bind one guarded Friday objective | objective remains canonical autonomy/task state | PROJECTS prepares and recovers objective; Objectives owns later gates | CF link + autonomy/task stores | planning, approval, isolation, execution and cancellation remain canonical | PROJECTS then Friday Objectives | deterministic API/frontend recovery coverage | INTEGRATED | complete real project build/validate/review E2E remains | qualify one bounded project lifecycle | 29, 48 | project completion E2E |
| 44. GitHub portfolio evidence | qualified CF evidence delegates to authenticated promotion gateway | no natural publish command | PROJECTS reviews and approves; publication endpoint is not exposed in UI | CF + task/Git history | explicit owner approval plus `GITHUB_WRITE` auth and eligible task | PROJECTS approval, then authenticated gateway | deterministic publication binding/retry coverage | INTEGRATED | final owner-visible publish action and real E2E remain | surface existing authenticated publication without bypass | 43, 45 | CF-E2E-008 |
