...
| 36. Practice Lab | typed assignment/draft/run/attempt service over CF SQLite | explicit practice/resume capability handoff | editable highlighted Python Lab, Run/Test/Submit, output, hint, history/diffs | CF SQLite + disposable lab directory | fail-closed Bubblewrap, denied network, fixed Python command, bounded resources; no auto-mastery | open Practice Lab / practice this | real voice route + owner visual workspace/CLOSE passed; live Run/Test/Hint/Submit/restart/resume/evidence automation passed | QUALIFIED | Python-only; no selected-code/screen context | selected-code context later | 16, 27 | E2E-013 |
